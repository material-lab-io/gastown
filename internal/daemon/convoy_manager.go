...
type ConvoyManager struct {
	...
	// Values are time.Time (switched from int64 IDs when beads migrated events.id to UUID).
...
func (m *ConvoyManager) pollStore(name string, store beadsdk.Storage, stores map[string]beadsdk.Storage, seen map[string]bool) {
	// Load per-store high-water mark (time-based since events.id is now UUID)
